...
public:
    static void yx_simple_filter_init(){
        char * fname;
        ...
        do{
            if(fname != NULL){
                ...
                if(f == NULL){
                    LOG_WRN("failed to open stopword file: %s\n", fname);
                    break;
                }
                char buf[1024];
        ...
        LOG_INF("initialized stopwords filter module by Y.X. (stoplist_size: %ld, file: %s)\n",
            stoplist.size(),
    ...
    void yx_simple_filter(std::string & content, const std::string & uid){
        ...
        if(cache && !g_nostop){
            ...
            LOG_INF("cache stopword %s\n", ctx2);
        }
    }
    SWordsFilter(){
        yx_simple_filter_init();
    }
};
